    ...
    // Dentro da função handleCreate no arquivo Step.tsx
    ...
    try {
        ...
        const profileData = {
        name: data.name, // Nome no nível raiz do documento
        profile: {
        ...
        // DEPOIS (RECOMENDADO):
        // Envie os dados que a API precisa diretamente para a próxima tela
        router.push({
        pathname: "/create",
        params: {
            nome: data.name, // Use 'nome' se sua API espera esse campo
            peso: data.weight,
            idade: data.age,
            altura: data.height,
            // Adicione aqui outros campos que sua API possa precisar (sexo, objetivo, etc.)
            // que talvez já estejam salvos ou venham de telas anteriores.
        },
        });
    } catch (error) {
        console.log(error);
